...
const SECTIONS = ["top", "operator", "systems", "professional", "lab", "roots", "contact"];
...
await page.screenshot({ path: `${out}/08-hero-midscroll.png` });
console.log("ok desktop hero-midscroll");
await page.close();

/* ── desktop 1280x800 hero shot (face-clear check at a common laptop size) ── */
const p1280 = await browser.newPage({ viewport: { width: 1280, height: 800 } });
await p1280.goto(base, { waitUntil: "networkidle" });
await p1280.waitForTimeout(2600);
await p1280.screenshot({ path: `${out}/12-hero-1280.png` });
console.log("ok desktop hero 1280x800");
await p1280.close();
...
await m.screenshot({ path: `${out}/13-mobile-hero.png` });
...
await m.screenshot({ path: `${out}/14-mobile-midscroll.png` });
...
await m.screenshot({ path: `${out}/15-mobile-systems.png` });
console.log("ok mobile systems");

await mjump("contact");
await m.waitForTimeout(1600);
await m.screenshot({ path: `${out}/16-mobile-contact.png` });
