...
/**
 * KM Lab scene v3 — the actor troupe.
 *
...
function CoreActor() {
  ...
  useFrame(({ camera }, dt) => {
    ...
    im.emissiveIntensity = 0.3 + 0.4 * c.glow;
    ...
    <group ref={pos}><group ref={spin}>
      ...
      {[0, 1, 2].map((i) => (
        <mesh key={i} ref={(el) => { if (el) rings.current[i] = el; }}
          ...
          <meshBasicMaterial color={i === 1 ? "#9d6bff" : "#4be1ff"} transparent opacity={0.3} />
        </mesh>
      ))}
  ...
  return (
    <group ref={pos}><group ref={spin}>
      <instancedMesh ref={strandA} args={[undefined, undefined, HELIX_N]}>
        ...
        <meshStandardMaterial color="#0a0f1e" emissive="#4be1ff" emissiveIntensity={1.1} roughness={0.3} metalness={0.7} />
      </instancedMesh>
      <instancedMesh ref={strandB} args={[undefined, undefined, HELIX_N]}>
        <sphereGeometry args={[0.075, 10, 10]} />
        <meshStandardMaterial color="#0a0f1e" emissive="#9d6bff" emissiveIntensity={1.1} roughness={0.3} metalness={0.7} />
      </instancedMesh>
      <instancedMesh ref={rungs} args={[undefined, undefined, 11]}>
        <cylinderGeometry args={[0.016, 0.016, 1, 6]} />
        <meshBasicMaterial color="#4be1ff" transparent opacity={0.35} />
      </instancedMesh>
    </group></group>
  );
}
  ...
  return (
    <group ref={pos}><group ref={spin}>
      <lineSegments geometry={edges}>
        <lineBasicMaterial color="#4be1ff" transparent opacity={0.3} />
      </lineSegments>
      <instancedMesh ref={nodes} args={[undefined, undefined, verts.length]}>
        <icosahedronGeometry args={[0.11, 0]} />
        <meshStandardMaterial color="#0a0f1e" emissive="#9d6bff" emissiveIntensity={1.2} roughness={0.3} metalness={0.8} flatShading />
      </instancedMesh>
      <mesh ref={core}>
        <icosahedronGeometry args={[0.34, 1]} />
        <meshStandardMaterial color="#0a0f1e" emissive="#4be1ff" emissiveIntensity={1} roughness={0.3} metalness={0.85} flatShading />
      </mesh>
    </group></group>
  );
}
  ...
  return (
    <group ref={pos}><group ref={spin}><group ref={tilt}>
      <mesh geometry={flaskGeo}>
        <meshBasicMaterial color="#4be1ff" transparent opacity={0.1} side={2} />
      </mesh>
      <mesh geometry={flaskGeo}>
        <meshBasicMaterial color="#4be1ff" wireframe transparent opacity={0.16} />
      </mesh>
      <mesh ref={liquid} position={[0, -0.72, 0]}>
        <coneGeometry args={[0.55, 0.62, 22]} />
        <meshStandardMaterial color="#1a0f2e" emissive="#9d6bff" emissiveIntensity={1} roughness={0.25} metalness={0.4} transparent opacity={0.85} />
      </mesh>
      <points ref={bubblePts} geometry={bubbles.geo}>
        <pointsMaterial color="#9d6bff" size={0.05} transparent opacity={0.8} depthWrite={false} />
      </points>
      <points ref={sprayPts} geometry={spray.geo}>
        <pointsMaterial color="#4be1ff" size={0.06} transparent opacity={0.85} depthWrite={false}
    ...
    <group ref={pos}><group ref={spin}>
      <group ref={ship}>
        <mesh position={[0, 0.62, 0]}>
          ...
          <meshStandardMaterial color="#0a0f1e" emissive="#4be1ff" emissiveIntensity={0.9} roughness={0.3} metalness={0.85} />
        </mesh>
        <mesh>
        ...
        <mesh position={[0, 0.1, 0.2]} rotation={[Math.PI / 2, 0, 0]}>
          ...
          <meshStandardMaterial color="#0a0f1e" emissive="#9d6bff" emissiveIntensity={1.2} roughness={0.3} metalness={0.8} />
        </mesh>
        <mesh position={[0, -0.52, 0]}>
          <cylinderGeometry args={[0.13, 0.2, 0.22, 16]} />
          <meshStandardMaterial color="#0a0f1e" emissive="#ffb454" emissiveIntensity={0.7} roughness={0.4} metalness={0.8} />
        </mesh>
        {[0, 1, 2].map((i) => (
          <mesh key={i} position={[Math.cos((i / 3) * Math.PI * 2) * 0.22, -0.42, Math.sin((i / 3) * Math.PI * 2) * 0.22]}
            ...
            <meshStandardMaterial color="#0a0f1e" emissive="#4be1ff" emissiveIntensity={0.5} roughness={0.35} metalness={0.85} />
          </mesh>
        ))}
      </group>
      <points ref={exhaustPts} geometry={exhaust.geo}>
        <pointsMaterial color="#ffb454" size={0.075} transparent opacity={0.85} depthWrite={false}
          blending={THREE.AdditiveBlending} />
      </points>
    </group></group>
  );
...
function AtomActor() {
  ...
  return (
    <group ref={pos}><group ref={spin}>
      <mesh>
        <icosahedronGeometry args={[0.34, 1]} />
        <meshStandardMaterial color="#0a0f1e" emissive="#ffb454" emissiveIntensity={1.1} roughness={0.3} metalness={0.85} flatShading />
      </mesh>
      {ORBITS.map((q, i) => (
        <mesh key={i} quaternion={q}>
          <torusGeometry args={[1.15, 0.012, 8, 80]} />
          <meshBasicMaterial color={i === 1 ? "#9d6bff" : "#4be1ff"} transparent opacity={0.35} />
        </mesh>
      ))}
      {[0, 1, 2].map((i) => (
        <mesh key={`e${i}`} ref={(el) => { if (el) electrons.current[i] = el; }}>
          <sphereGeometry args={[0.09, 12, 12]} />
          <meshStandardMaterial color="#0a0f1e" emissive={i === 1 ? "#9d6bff" : "#4be1ff"} emissiveIntensity={1.6} roughness={0.3} metalness={0.7} />
        </mesh>
      ))}
    </group></group>
  );
    ...
    <group ref={pos}><group ref={spin}>
      ...
      <group ref={bat} position={[0.55, 0.55, 0]}>
        <mesh position={[0, -0.24, 0]}>
          ...
          <meshStandardMaterial color="#141b30" emissive="#ffb454" emissiveIntensity={0.25} roughness={0.5} metalness={0.6} />
        </mesh>
        <mesh position={[0, -0.98, 0]}>
          <boxGeometry args={[0.26, 1.05, 0.1]} />
          <meshStandardMaterial color="#0a0f1e" emissive="#ffb454" emissiveIntensity={0.55} roughness={0.4} metalness={0.7} />
        </mesh>
      </group>
      {/* ball with seam */}
      <group ref={ball}>
        <mesh>
          ...
          <meshStandardMaterial color="#2e0a14" emissive="#ff5c8a" emissiveIntensity={1.1} roughness={0.35} metalness={0.5} />
        </mesh>
        <mesh rotation={[Math.PI / 2.4, 0.4, 0]}>
          <torusGeometry args={[0.16, 0.012, 6, 32]} />
          <meshBasicMaterial color="#ffb454" transparent opacity={0.8} />
        </mesh>
      </group>
    </group></group>
  );
  ...
  return (
    <group ref={pos}><group ref={spin}>
      <mesh ref={heart}>
        ...
        <meshStandardMaterial color="#0a0f1e" emissive="#4be1ff" emissiveIntensity={1.2} roughness={0.25} metalness={0.85} flatShading />
      </mesh>
      {[0, 1, 2].map((i) => (
        <mesh key={i} ref={(el) => { if (el) rings.current[i] = el; }}>
          <torusGeometry args={[1, 0.02, 8, 64]} />
          <meshBasicMaterial color={i === 1 ? "#9d6bff" : "#4be1ff"} transparent opacity={0.4} />
        </mesh>
      ))}
      {[0, 1, 2, 3, 4].map((i) => {
        ...
        return (
          <mesh key={`d${i}`} position={[Math.cos(a) * 0.85, Math.sin(a) * 0.85, 0]}>
            ...
            <meshBasicMaterial color="#ffb454" transparent opacity={0.7} />
          </mesh>
        );
      })}
    </group></group>
  ...
  return (
    <>
      <mesh ref={seed} visible={false}>
        ...
        <meshBasicMaterial color="#4be1ff" />
      </mesh>
      <mesh ref={halo} visible={false}>
        <sphereGeometry args={[1, 16, 16]} />
        <meshBasicMaterial color="#4be1ff" transparent opacity={0.14} blending={THREE.AdditiveBlending} depthWrite={false} />
      </mesh>
    </>
  );
}
