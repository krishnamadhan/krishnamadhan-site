...
export const CHAPTERS: Chapter[] = [
  ...
  { id: "map",       section: "work",     label: "CH.03 / SYSTEMS",       x: 3.0,  y: 0.6,  scale: 0.55,  glow: 0.7,  warmth: 0,    spread: 1.0, flat: 0.6,  split: 0.85, squash: 1,   spin: 0.7,  pulse: 0 , dim: 0.25 },
  ...
  { id: "orbital",   section: "skills",   label: "CH.06 / STACK",         x: -3.0, y: 0.5,  scale: 0.6, glow: 0.7,  warmth: 0.2,  spread: 1.0, flat: 0.9,  split: 0.9, squash: 1,    spin: 0.6,  pulse: 0 , dim: 0.22 },
...
export function updateChapterFromScroll() {
  ...
  const raw = next === i ? 0 : Math.min(Math.max((centre - tops[i]) / span, 0), 1);
  // hold each chapter's state for most of its section; morph only in the
  // final 28% approach to the next section — otherwise a right→left position
  // change sweeps the object straight through the content mid-section.
  const ZONE = 0.72;
  ...
  const idx = i;   // label = section containing the centre — no early switch
